...
def hold_out_seeds():
    ...
    # stages, stage_vecs = trainer.api(test_data_loader)
    # with open('./hold_out_seeds.txt', 'w') as fout:
    ...
    # with open(os.path.join(output_folder, 'additional_graphs.txt'), 'w') as fout:
    #     for i, g in enumerate(labeled_dataset.graphs):
    #         g["pred"] = stages[i]
    #         g["stage_vec"] = stage_vecs[i]
    #         fout.write(json.dumps(g))
...
# labeled_data_loader 的 bert input 每次不一样，应该是vocab的问题
    ...
    for epoch in range(args.epochs):
        ...
        with open(os.path.join(output_folder, 'graphs_{}.txt'.format(epoch)), 'w') as fout:
            for i, g in enumerate(labeled_dataset.graphs):
                g["pred"] = stages[i]
                g["stage_vec"] = stage_vecs[i]
                fout.write(json.dumps(g))
