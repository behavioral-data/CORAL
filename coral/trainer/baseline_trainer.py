...
class BaselineTrainer:
    ...
    def __init__(self, word2vec, vocab,
                 train_dataloader: DataLoader, test_dataloader: DataLoader,
                 lr: float = 1e-4, n_hidden = 256,
        ...
        self.model = Single(self.vocab, self.word2vec, n_topics=n_topics, n_hidden = n_hidden).to(self.device)
        ...
        with torch.no_grad():
            for i, item in data_iter:
                ...
                reconstructed_vec, graph_vec, origin_neg, topic_dist, stage_vec = self.model.forward(
                    data["word2vec_embedding"], ndata["word2vec_embedding"], data["segment_label"], ndata["segment_label"], data["adj_mat"], ndata["adj_mat"])
